...
//
			...
			else
			{
				foreach (var f in __result)
				{
					for (int i = 0; i < f.categories.Count; ++i)
					{
						string c = f.categories[i];
						if (c == "Civil")
							__result[i].categories.Add("REBCivil");
						if (c == "Raider")
							__result[i].categories.Add("REBRaider");
						if (c == "Slave")
							__result[i].categories.Add("REBSlave");
						if (c == "Trader")
							__result[i].categories.Add("REBTrader");
						if (c == "Traveler")
							__result[i].categories.Add("REBTraveler");
						if (c == "Tribal") 
							__result[i].categories.Add("REBTribal");
					}
				}
			}
		}
	...
	static class Patch_PawnBioAndNameGenerator_GiveAppropriateBioAndNameTo
	{
		...
		static bool Prefix(Pawn pawn, string requiredLastName, FactionDef factionType) {
			List<string> backstoryCategories = GetBackstoryCategoryFiltersForFI.Invoke(null, new object[] { pawn, factionType }) as List<string>;
